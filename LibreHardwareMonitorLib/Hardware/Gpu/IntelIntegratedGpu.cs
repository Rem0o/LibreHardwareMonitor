...
internal class IntelIntegratedGpu : GenericGpu
{
    ...
    private readonly IntelMsr _intelMsr;
               ...
               settings)
    {
        ...
        if (_intelMsr.ReadMsr(MSR_PP1_ENERGY_STATUS, out uint eax, out uint _))
        {
    ...
    public override void Update()
    {
        if (D3DDisplayDevice.GetDeviceInfoByIdentifier(_deviceId, out D3DDisplayDevice.D3DDeviceInfo deviceInfo))
        {
            ...
            if (_powerSensor != null && _intelMsr.ReadMsr(MSR_PP1_ENERGY_STATUS, out uint eax, out uint _))
            {
